...
router.post('/', upload.single('Image'), function(req, res) {
    const { ItemName, username, Category, Price } = req.body;
    ...
    console.log(Image);
    // 检查信息是否完整
    if (!ItemName || !username || !Category || !Price ) {
    ...
    pool.query('SELECT userID FROM user WHERE userName = ?', [username], function(error, results) {
        ...
        if (results.length === 0) {
            ...
            // 将商品信息插入Items表
            pool.query('INSERT INTO Items (ItemName, userID, Category, Price, Image) VALUES (?, ?, ?, ?, ?)',
                [ItemName, userID, Category, Price, Image], function(error, results) {
                    if (error) {
                        return res.status(500).json({success: false, message: '插入Items表错误'});
                    }
                    res.json({success: true, message: '商品信息已成功入库'});
                });
        }
    });
});
